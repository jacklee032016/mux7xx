...
int i;
	...
	i2c_addr[0] = switch_addr>>0;
	i2c_addr[1] = switch_addr>>8;
	i2c_addr[2] = switch_addr>>16;

	msg[0].addr  = (RTL8305 | DEVICE_ID);
	msg[0].flags = 0;
	msg[0].len   = 3;
	msg[0].buf   = i2c_addr;
	msg[1].addr  = (RTL8305 | DEVICE_ID);
	...
	if (reg_val != NULL) {
		*reg_val = 0;
		for (i = 0; i < 4; i++)  { *reg_val = (i2c_data[i]<<(i*8)) | *reg_val; } 
	}
...
struct 	_buffer in;
	...
	if (reg_val != NULL) {
		*reg_val = 0;
		for (i = 0; i < 4; i++)  { *reg_val = (data[i]<<(i*8)) | *reg_val; } 
	}
#endif	
	return RT_ERR_OK;
}
